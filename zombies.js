  ...
  if(itemToEquip instanceof Weapon){
    if(this._pack.indexOf(itemToEquip)>-1){
      if(this.equipped === false){
        ...
        console.log(this.name + ` equipped ${itemToEquip.name} even though their kicks are the greatest weapon of all time.`);
        return true; 
      }
      if(this.equipped !== false){
...
useItem(item){
  if(item instanceof Weapon || item instanceof Food){
    if(item instanceof Weapon){
      this.equip(item);
      console.log(this.name + " just equipped: " + item.name);
    }
    if(item instanceof Food){
      this.eat(item);
      console.log(this.name + " just nom'd on: " + item.name);
    }
  }else{
